...
describe('x-ripple', () => {
  ...
  it('should update element property reactively', async() => {
    const wrapper = mount({
      ...
      template: '<div class="a" x-ripple="ripple" />',
    });
  ...
  it('should trigger ripple on mousedown', () => {
    ...
    expect(wrapper.find('.a-ripple__container').exists()).toBe(true);

    const mouseupEvent = new MouseEvent('mouseup');
    wrapper.element.dispatchEvent(mouseupEvent);

    jest.runAllTimers();
    expect(wrapper.find('.a-ripple__container').exists()).toBe(false);
  });
  ...
  it('should only ripple on one element', () => {
    const wrapper = mount({
      directives: { Ripple },
      template: '<div a-ripple><div class="child" a-ripple></div></div>',
    });
    ...
    expect(wrapper.findAll('.a-ripple__container')).toHaveLength(1);

    const mouseupEvent = new MouseEvent('mouseup', { detail: 1, bubbles: true });
    child.dispatchEvent(mouseupEvent);

    jest.runAllTimers();
    expect(wrapper.findAll('.a-ripple__container')).toHaveLength(0);
  });
