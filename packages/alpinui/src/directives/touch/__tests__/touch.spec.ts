...
describe('x-touch', () => {
  const mountFunction = (value: TouchValue): Element => {
    const wrapper = mount({
      directives: { Touch: TouchDirective },
      props: {
        value: Object,
      },
      template: '<div class="test" x-touch="value" />',
    }, { props: { value } });
  ...
  it('should unmount', async() => {
    ...
    const wrapper = mount({
      ...
      template: '<div x-if="bound" class="test" x-touch="value" /><div x-if="!bound" class="test" />',
    }, {
      props: {
        value: { start },
        bound: true,
