...
function checkEvent(
  e: MouseEvent,
  el: HTMLElement,
  binding: HeadlessDirectiveBinding<ClickOutsideValue>,
): boolean {
...
export const ClickOutsideDirective: HeadlessDirective<ClickOutsideValue> = {
  ...
  mounted(
    ...
    const onMousedown = (e: Event) => {
      el._clickOutsideMeta = el._clickOutsideMeta || { lastMousedownWasOutside: false };
      el._clickOutsideMeta.lastMousedownWasOutside = checkEvent(e as MouseEvent, el, binding);
    };
  ...
  unmounted(
    ...
    handleShadow(el, (app: HTMLElement) => {
      ...
      const { onClick, onMousedown } = el._clickOutside.get(el)!;
