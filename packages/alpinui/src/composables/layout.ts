...
import { convertToUnit, eagerComputed, findChildrenWithProvide } from '@/util/helpers';
import { propsFactory } from '@/util/propsFactory';

// Types
import type { AlpineInstance, Data } from 'alpine-composition';
import type { Ref } from 'alpine-reactivity';
import type { CSSProperties, DeepReadonly, InjectionKey, Prop } from 'vue';
...
export const AlpinuiLayoutKey: InjectionKey<LayoutProvide> = Symbol.for('alpinui:layout');
export const AlpinuiLayoutItemKey: InjectionKey<{ id: string }> = Symbol.for('alpinui:layout-item');
...
export function useLayout(vm: AlpineInstance<Data>) {
  const layout = vm.$inject(AlpinuiLayoutKey);

  if (!layout) throw new Error('[Alpinui] Could not find injected layout');

  const layoutIsReady = vm.$nextTick();
...
export function useLayoutItem(
  ...
  vm.$provide(AlpinuiLayoutItemKey, { id });
  ...
  vm.$onBeforeUnmount(() => layout.unregister(id));
...
export function createLayout(
  ...
  const parentLayout = vm.$inject(AlpinuiLayoutKey, null);
  ...
  rootVm.$provide(AlpinuiLayoutKey, {
    register: (
      vm,
      {
        id,
      ...
      const instances = findChildrenWithProvide(AlpinuiLayoutItemKey, rootVm?.vnode);
      ...
      const layoutItemStyles = computed<CSSProperties>(() => {
        ...
        if (!item) throw new Error(`[Alpinui] Could not find layout item "${id}"`);
