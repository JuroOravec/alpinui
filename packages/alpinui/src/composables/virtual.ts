...
import { clamp, debounce } from '@/util/helpers';
...
export function useVirtual <T>(
  ...
  const { computed, nextTick, ref, shallowRef, watchEffect, watch, onBeforeUnmount } = vm.reactivity;

  const display = useDisplay(vm);
