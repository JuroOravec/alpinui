...
import type { App, ComputedRef, DeepReadonly, InjectionKey, Ref } from 'vue';
...
export interface ThemeInstance {
  ...
  readonly themeClasses: ComputedRef<ClassValue>;
...
export function createTheme(
  ...
  options?: ThemeOptions,
    ...
    installHead: (head?: Unhead) => void;
  } {
  const { computed, ref, watch, watchEffect } = vm.reactivity;
  ...
  function installHead(head?: Unhead | HeadClient) {
    if (parsedOptions.isDisabled) return;
  ...
  const themeClasses = computed((): ClassValue => ({
...
export function provideTheme(
  ...
  const { computed, inject, provide } = vm.reactivity;
...
export function useTheme(vm: HeadlessInstance) {
