// Utilities
import { computed, ref, watch, watchEffect } from 'alpine-reactivity';
import {
  darken,
  getForeground,
  getLuma,
...
import { createRange, mergeDeep } from '@/util/helpers';
...
export interface ThemeInstance {
  ...
  readonly themeClasses: ComputedRef<Record<string, boolean>>;
  readonly styles: ComputedRef<string>;
...
export const ThemeSymbol: InjectionKey<ThemeInstance> = Symbol.for('alpinui:theme');
...
// Composables
export function createTheme(options?: ThemeOptions): ThemeInstance & { install: (head?: Unhead) => void } {
  ...
  function install(head?: Unhead) {
    ...
    if (head) {
      if (head.push) {
        ...
        if (IN_BROWSER) {
          (head as any).addHeadObjs(getHead());
          watchEffect(() => (head as any).updateDOM());
        } else {
          (head as any).addHeadObjs(getHead());
        }
      }
      ...
      let styleEl = IN_BROWSER
        ? document.getElementById('alpinui-theme-stylesheet')
        : null;
...
export function useTheme(vm: AlpineInstance<Data, Data, EmitsOptions>) {
  const theme = vm.$inject(ThemeSymbol, null);
