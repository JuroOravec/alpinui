...
export function createGroupBy(vm: HeadlessInstance, props: GroupProps) {
  const groupBy = useProxiedModel(vm, props, 'groupBy');

  return { groupBy };
}

export function provideGroupBy(
  vm: HeadlessInstance,
  options: {
    groupBy: Ref<readonly SortItem[]>;
  ...
  const { computed, provide, ref } = vm.reactivity;
...
export function useGroupBy(vm: HeadlessInstance) {
