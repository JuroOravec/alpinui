...
export function providePagination(
  vm: HeadlessInstance,
  options: {
    page: Ref<number>;
    itemsPerPage: Ref<number>;
  ...
  const { computed, provide, watchEffect } = vm.reactivity;
...
export function usePagination(vm: HeadlessInstance) {
...
export function usePaginatedItems <T>(
  ...
  watch(paginatedItems, (val) => {
    vm.emit('update:currentItems', ...([val] as any as never));
  });
