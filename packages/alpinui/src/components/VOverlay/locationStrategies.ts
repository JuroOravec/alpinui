...
import { nullifyTransforms } from '@/util/animation';
import { Box, getOverflow, getTargetBox } from '@/util/box';
import { consoleError } from '@/util/console';
import { getScrollParents } from '@/util/getScrollParent';
import { IN_BROWSER } from '@/util/globals';
import { clamp, convertToUnit, destructComputed } from '@/util/helpers';
import { isFixedPosition } from '@/util/isFixedPosition';
...
export function useLocationStrategies(
  ...
  const { onBeforeUnmount, ref, watch } = vm.reactivity;
  ...
  if (IN_BROWSER) {
    ...
    watch(
      [
        ...
        () => !!(data.isActive.value && props.locationStrategy),
      ],
      ([strategy, cond], [oldStrategy]) => {
...
function connectedLocationStrategy(
  ...
  const { computed, nextTick, watch, onBeforeUnmount } = vm.reactivity;
  ...
  const { preferredAnchor, preferredOrigin } = destructComputed(vm, () => {
